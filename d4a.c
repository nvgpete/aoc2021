...
int card[125][5][5],nbrs[100],x,nc=0,last,i,j,i2,j2,sum,found;
FILE *f;
char s[1000],*s2,c;
int n=0,cc;
f=fopen("d4a.txt","r");
...
//  printf("%d\n",nc);
//  for (i=0;i<5;i++) {
//    for (j=0;j<5;j++) printf("%2d  ",card[2][i][j]);
//    printf("\n");
//  }
  for (n=0;n<last;n++) {
    x=nbrs[n];
        ...
        for (j=0;j<5;j++) {
          if (card[cc][i][j]==x) {
            ...
            if (card[cc][i][0]+card[cc][i][1]+card[cc][i][2]+card[cc][i][3]+card[cc][i][4]==500 ||
              ...
              sum=0;
              for (i2=0;i2<5;i2++) for (j2=0;j2<5;j2++) if (card[cc][i2][j2]!=100) sum+=card[cc][i2][j2];
              printf("%d\n",sum*x);
              exit(0);
            }
            found=1;
